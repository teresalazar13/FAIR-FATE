...
def plot_avg_results(dataset_name, num_runs):
    fls = ["fedavg_alpha-0.25", "fedavg_gr_alpha-0.25", "fedavg_lr_alpha-0.25"]
    metrics = ["TPR_ratio"]
    ...
    for beta in [0.7, 0.8, 0.9, 0.99]:
        for lambda_exponential in [0.04, 0.045, 0.05]:
            fl = "fair_fate_l_e{}_b_{}_TPR_alpha-0.25".format(str(lambda_exponential), str(beta))
    ...
    dfs_fedmom = []
    for beta in [0.7, 0.8, 0.9, 0.99]:
        fl = "fedmom_b_{}_alpha-0.25".format(str(beta))
    ...
    plot_results(dfs, fls, './datasets/{}/rounds_plot_alpha-0.25.png'.format(dataset_name), metrics_results)
    get_last_round_plot(dfs, fls, './datasets/{}/last_round_plot_alpha-0.25.png'.format(dataset_name), metrics_results)
    plot_pareto_front(dfs_fair_fate_exp, fls_fair_fate_exp, './datasets/{}/pareto_front_alpha-0.25.png'.format(dataset_name), "ACC", "TPR_ratio")

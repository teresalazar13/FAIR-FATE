...
class FederatedLearningAlgorithm:
    ...
    def iterate(self, dataset, federated_train_data, n_features, x_val, y_val, x_test, y_test, clients_data_size):
        self.state, weights = self.algorithm.next(self.state, federated_train_data)
        self.state, model = self.update(weights, n_features, x_val, y_val, clients_data_size)
